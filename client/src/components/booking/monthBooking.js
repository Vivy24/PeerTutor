...
const MonthBooking = ({ meetings, loggedInUser, tutor, reload }) => {
  ...
  const bookMonthly = async (event) => {
    ...
    await bookedHour.forEach(async (bookedHour) => {
      ...
      try {
        await axios.post("/api/meetings", body, config).then((res) => {
          setBookingError([]);
          setSuccess(res.data);
        });
      } catch (error) {
  ...
  return (
    <Fragment>
      ...
      <form onSubmit={bookMonthly} className={styles.bookingForm}>
        {success && (
        ...
        <select onChange={chooseMonth} id="monthlyBook" class="form-control">
              ...
              <div class="row align-items-center mb-2">
                <div class="col-5">
                  <input
                    ...
                    onChange={chooseWeekDayEvent}
                  />
                  <label class="form-select-label " for="defaultCheck1">
                    Sunday
                  </label>
                </div>
                <div class="col-7">
